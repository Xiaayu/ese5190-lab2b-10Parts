...
void render_to_consol(Repl status, char input) {
    printf("Current register: 0x%08x, Register value: 0x%08x, Write: 0x%08x, counter: %d, counter_value: %d, time: %d\n",  
    status.target_address, status.value, status.write, status.counter, status.counter_value, status.time);
}  
